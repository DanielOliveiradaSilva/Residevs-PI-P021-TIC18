...
void ctrl_biblioteca::insere_livro()
{
    ...
    do {
        ...
        if ( titulo.empty() ) {
            cout << "Aviso: Entre com um título." << endl;
        }
    } while ( titulo.empty() );
        ...
        if (localizarLivro(titulo, indiceLivro) && verificarDisponibilidadeDelivro(titulo))
        {
            ...
            do {
                ...
                if ( !Data::isDataValida(data) ) {
                    cout << "Aviso: Entre com uma data válida no formato dd/mm/aaaa." << endl;
                }
            } while ( !Data::isDataValida(data) ); // verificar se é data válida
